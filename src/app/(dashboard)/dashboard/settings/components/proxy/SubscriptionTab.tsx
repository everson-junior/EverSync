        ...
        <div className="rounded-lg border border-border bg-surface p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-1 text-sm">
              ...
              />
              <span className="text-xs text-text-muted">
                {t("proxySubscription.localCoreEndpointHint")}
              </span>
            </label>
          </div>
                ...
                <div className="min-w-0">
                  ...
                  {showCoreHint && (
                    <div className="mt-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 space-y-1.5">
                      <p className="font-medium">
                        {t("proxySubscription.needsCoreTitle", { count: needsCoreNodes.length })}
                      </p>
                      <p>{t("proxySubscription.needsCoreDescription")}</p>
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    ...
                    {needsCoreNodes.length > 0
                      ...
                      : ""}
                    {sub.lastFetchedAt
                      ? ` · ${t("proxySubscription.lastSynced", { value: sub.lastFetchedAt })}`
                      : ""}
                    {sub.consecutiveFailures > 0
                      ? ` · ${t("proxySubscription.consecutiveFailures", { count: sub.consecutiveFailures })}`
                      : ""}
                    {sub.lastErrorAt
                      ? ` · ${t("proxySubscription.lastError", { value: sub.lastErrorAt })}`
                      : ""}
                  </p>
                </div>
