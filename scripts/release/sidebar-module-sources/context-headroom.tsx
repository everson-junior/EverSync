...
export default function ContextHeadroomModule() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getHost().fetch("/api/context/analytics/engine?engineId=headroom&days=7").then(asJson),
    ])
      .then(([enginesData, settingsData, analyticsData]) => {
        if (cancelled) return;
        const found = (enginesData?.engines as Engine[] | undefined)?.find(
          (candidate) => candidate.id === "headroom"
        );
        ...
        setConfig({ ...defaults, ...(settingsData?.headroom ?? {}) });
        setAnalytics(analyticsData as Analytics | null);
      })
      .catch(() => {
        if (!cancelled) setMessage("Unable to load Headroom settings.");
      })
      .finally(() => {
  ...
  const save = async () => {
    ...
    try {
      const response = await getHost().fetch("/api/settings/compression", {
        ...
        body: JSON.stringify({ headroom: config }),
      });
  ...
  if (loading) return <p className="p-6 text-sm text-text-muted">Loading Headroom...</p>;
  if (!engine) return <p className="p-6 text-sm text-text-muted">Headroom is unavailable.</p>;
