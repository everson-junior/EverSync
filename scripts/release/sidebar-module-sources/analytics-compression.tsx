...
export default function AnalyticsCompressionModule() {
  ...
  if (!stats)
    return (
      <p className="p-6 text-sm text-text-muted">{error ?? "Loading compression analytics..."}</p>
    );
  const total = stats.totalRequests + (stats.totalSkipped ?? 0);
  const cards = [
    ["Requests", total],
    ["Tokens saved", stats.totalTokensSaved],
    ["Average savings", `${stats.avgSavingsPct}%`],
    ["Average duration", `${stats.avgDurationMs}ms`],
    ["Validation fallbacks", stats.validationFallbacks],
  ];
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5 p-4">
      <header>
        <h1 className="text-2xl font-bold text-text-main">Compression Analytics</h1>
        <p className="mt-1 text-sm text-text-muted">
          Savings and execution statistics for the compression pipeline.
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Breakdown title="By mode" entries={stats.byMode} total={total} />
        <Breakdown title="By provider" entries={stats.byProvider} total={stats.totalRequests} />
      </section>
    </div>
  );
}
...
function Breakdown({
  ...
  title: string;
  entries: Record<string, { count: number; tokensSaved: number }>;
  total: number;
}) {
  return (
            ...
            <div key={name}>
              <div className="flex justify-between gap-3 text-sm">
                ...
                <span className="text-text-muted">
                  {data.count.toLocaleString()} requests, {data.tokensSaved.toLocaleString()} saved
                </span>
              </div>
              <div className="mt-1 h-2 overflow-hidden rounded bg-bg">
                <div
                  className="h-full rounded bg-primary"
                  style={{ width: `${total ? Math.round((data.count / total) * 100) : 0}%` }}
                />
              </div>
            </div>
          ))}
