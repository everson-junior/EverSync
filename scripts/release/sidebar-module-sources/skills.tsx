...
export default function SkillsModule() {
  ...
  useEffect(() => {
    getHost()
      .fetch("/api/skills?page=1&limit=100")
      .then(async (r) => {
        const d = await r.json();
        if (!r.ok) throw new Error(d.error || `HTTP ${r.status}`);
        setData(d);
      })
      .catch((e: unknown) => setError(e instanceof Error ? e.message : "Unable to load skills."));
  }, []);
  return (
    <main className="mx-auto max-w-5xl space-y-4 p-4">
      <h1 className="text-2xl font-bold text-text-main">Skills</h1>
      {data ? (
