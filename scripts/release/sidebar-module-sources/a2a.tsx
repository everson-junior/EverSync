...
export default function A2aModule() {
  ...
  useEffect(() => {
    getHost()
      ...
      .catch((e: unknown) =>
        setError(e instanceof Error ? e.message : "Unable to load A2A status.")
      );
  }, []);
  return (
    <main className="mx-auto max-w-5xl space-y-4 p-4">
      <h1 className="text-2xl font-bold text-text-main">A2A</h1>
      {data ? (
        <pre className="max-h-[36rem] overflow-auto rounded border border-border bg-surface p-4 text-xs">
          {JSON.stringify(data, null, 2)}
        </pre>
      ) : (
        <p className="text-sm text-text-muted">Loading A2A status...</p>
      )}
