...
export default function BatchModule() {
  ...
  const load = () =>
    getHost()
      ...
      .catch((e: unknown) => setError(e instanceof Error ? e.message : "Unable to load batches."));
  useEffect(() => {
    load();
  }, []);
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4">
      <header className="flex justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-main">Batches</h1>
          <p className="text-sm text-text-muted">OpenAI-compatible batch processing jobs.</p>
        </div>
        <button
