...
test("a clean installation discovers the latest strict release from its manifest", async () => {
  await withPluginDataDir(async () => {
    ...
    const manifest = {
      releaseVersion,
      sourceMode: "strict",
      assets: PLUGIN_CATALOG.map((entry) => ({
        id: entry.id,
        route: entry.route,
        file: `${entry.id}-${releaseVersion}.mjs`,
        version: releaseVersion,
        sha256: entry.id === plugin.id ? checksum(releaseBody) : "f".repeat(64),
      })),
    };
    const requestedUrls: string[] = [];

    const installed = await installPluginBundle(plugin, async (url) => {
      requestedUrls.push(String(url));
      return new Response(requestedUrls.length === 1 ? JSON.stringify(manifest) : releaseBody);
    });

    assert.equal(installed.version, releaseVersion);
    assert.equal(installed.checksum, checksum(releaseBody));
    assert.deepEqual(requestedUrls, [
      "https://github.com/everson-junior/EverSync/releases/latest/download/manifest.json",
      `https://github.com/everson-junior/EverSync/releases/download/v${releaseVersion}/mcp-${releaseVersion}.mjs`,
    ]);
    assert.deepEqual((await readVerifiedInstalledPlugin(plugin))?.body, releaseBody);
  });
});
